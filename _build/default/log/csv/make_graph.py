...
data_name = [['nofilter','noinvmap','noinvmap&nofilter','Basic'],['^','v','D','o'],['g','m','r','b'],['none','none','none','none']]
...
# set label
# plt.title(typ.upper())
plt.title("Reference Implementation")
...
# set data
for i in range(datas):
    plt.plot(globals()['time_{}'.format(i+1)], label=data_name[0][i]+' (solved = '+str(globals()['time_{}'.format(i+1)].count())+')', color=data_name[2][i], marker=data_name[1][i], markersize=10, markevery=10, fillstyle=data_name[3][i])
