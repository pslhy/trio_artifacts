...
let rec node_of_vsa vsa =
	match vsa with
	| Direct expr -> 1
	| Join (_, vsa_lst) -> 1 + (List.fold_left (fun acc vsa -> acc + (node_of_vsa vsa)) 0 vsa_lst)
	| Union vsa_set -> 1 + BatSet.fold (fun vsa acc -> (node_of_vsa vsa) + acc) vsa_set 0
	| Empty -> 0
