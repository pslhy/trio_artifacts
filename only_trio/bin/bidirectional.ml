...
let synthesis spec =
	...
	let rec iter depth (ty_to_exprs, ty_to_sigs, sig_to_expr) =
		...
		try
			...
			in
			let _ = print_endline (Printf.sprintf "vsa nodes: %d" (list_fold (fun x y -> (node_of_vsa x)+y) !trace_vsas 0)) in
			let _ = print_endline (Printf.sprintf "vsa pgm sum: %d" (list_fold (fun x y -> (pgm_count_of_vsa x)+y) !trace_vsas 0)) in
			wrap spec sol 
